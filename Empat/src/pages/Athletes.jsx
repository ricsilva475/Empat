...
import { SPORTS, SKILL_MAP } from "../constants";
...
export default function Athletes() {
  ...
  const submit = async (e) => {
    e.preventDefault();
    try {
      await api.post("/athletes", { ...form, age: parseInt(form.age) });
  ...
  return (
    <div className="space-y-6" data-testid="athletes-page">
      ...
      {showForm && (
        <form onSubmit={submit} className="rounded-2xl bg-white border border-slate-200 p-6 grid md:grid-cols-2 gap-4" data-testid="athlete-form">
          <div>
            <label className="text-sm font-medium text-slate-700">Nome</label>
            <input required value={form.name} onChange={e=>setForm({...form,name:e.target.value})} className="mt-1.5 w-full px-4 py-2.5 rounded-xl border border-slate-200" data-testid="athlete-name"/>
          </div>
          <div>
            <label className="text-sm font-medium text-slate-700">Idade</label>
            <input type="number" min={5} max={25} required value={form.age} onChange={e=>setForm({...form,age:e.target.value})} className="mt-1.5 w-full px-4 py-2.5 rounded-xl border border-slate-200" data-testid="athlete-age"/>
          </div>
          <div>
            <label className="text-sm font-medium text-slate-700">Desporto</label>
            <select value={form.sport} onChange={e=>setForm({...form,sport:e.target.value})} className="mt-1.5 w-full px-4 py-2.5 rounded-xl border border-slate-200 bg-white" data-testid="athlete-sport">
          ...
          <div>
            ...
            <input value={form.team} onChange={e=>setForm({...form,team:e.target.value})} className="mt-1.5 w-full px-4 py-2.5 rounded-xl border border-slate-200" placeholder="Sub-12 A" data-testid="athlete-team"/>
          </div>
          <div>
            <label className="text-sm font-medium text-slate-700">Posição (opcional)</label>
            <input value={form.position} onChange={e=>setForm({...form,position:e.target.value})} className="mt-1.5 w-full px-4 py-2.5 rounded-xl border border-slate-200" data-testid="athlete-position"/>
          </div>
          <div className="md:col-span-2">
            <label className="text-sm font-medium text-slate-700">Notas</label>
            <textarea value={form.notes} onChange={e=>setForm({...form,notes:e.target.value})} rows={2} className="mt-1.5 w-full px-4 py-2.5 rounded-xl border border-slate-200" data-testid="athlete-notes"/>
          </div>
